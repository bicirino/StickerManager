...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <span class="text-muted small">Clique em uma figurinha para marcar como obtida ou repetida (botão direito para desfazer) </span>
</div>
...
<form class="card card-body mb-3" method="get">
  ...
  <div class="row g-2">
    ...
    <div class="col-md-2"><select class="form-select" name="selecao">
    ...
    <div class="col-md-2"><select class="form-select" name="posicao">
        <option value="">Toda posição</option>
        <?php foreach ($posicoes as $p): ?><option value="<?= $p['id'] ?>" <?= ($_GET['posicao'] ?? '') == $p['id'] ? 'selected' : '' ?>><?= e($p['nome']) ?></option><?php endforeach; ?>
      </select></div>
    <div class="col-md-2"><select class="form-select" name="categoria">
...
$grupos = [];
foreach ($figs as $f) {
  $key = ($f['categoria_nome']) . ($f['selecao_nome'] ? ' — ' . $f['selecao_nome'] : '');
  $grupos[$key][] = $f;
}
